...
async function main() {
  ...
  await db.exec(`RESET ROLE;
    INSERT INTO skills(id,code,name,status) VALUES('${uid(900)}','CUSTOM_AI','AI Knowledge','ACTIVE');
    INSERT INTO courses(id,title,category,status) VALUES('5ad4db9e-9dfe-5478-870c-21ac5871fa5d','Handbook','Readiness','ACTIVE');
    INSERT INTO modules(id,course_id,title,skill_id,sequence,status) VALUES('${uid(901)}','5ad4db9e-9dfe-5478-870c-21ac5871fa5d','6.4: Technology and AI Fluency at Work',(SELECT id FROM skills WHERE code='ADAPT'),25,'ACTIVE'),('${uid(902)}','${uid(500)}','Practice module',(SELECT id FROM skills WHERE code='COMM'),1,'ACTIVE');
    INSERT INTO lessons(id,module_id,title,sequence,status) VALUES('${uid(903)}','${uid(902)}','Practice lesson',1,'ACTIVE');`);
  const progressBefore=await scalar('SELECT count(*) FROM lesson_progress');
  const scoresBefore=await scalar('SELECT count(*) FROM student_employability_scores');
  const learningSql=fs.readFileSync(path.join(migrationDir,'032_ordered_learning_content.sql'),'utf8');
  await db.exec(learningSql);
  check(await scalar(`SELECT skill_id FROM modules WHERE id='${uid(901)}'`),uid(900),'AI module connects to AI skill without recreating content');
  check(await scalar(`SELECT display_order FROM skills WHERE code='PROF'`),10,'professionalism appears first');
  await caller(uid(1));
  await db.exec(`UPDATE lessons SET practice_questions='[{"question":"What is a clear email?","answer":"A concise request."}]' WHERE id='${uid(903)}'`);
  await caller(uid(10));
  check((await scalar(`SELECT practice_questions FROM lessons WHERE id='${uid(903)}'`))[0].answer,'A concise request.','enrolled student can read practice content');
  await db.exec(`UPDATE lessons SET practice_questions='[]' WHERE id='${uid(903)}'`);
  await caller(uid(1));check((await scalar(`SELECT practice_questions FROM lessons WHERE id='${uid(903)}'`)).length,1,'student cannot change lesson practice content');
  await denied(`UPDATE lessons SET practice_questions='[{"question":"Missing answer"}]' WHERE id='${uid(903)}'`,'database rejects malformed practice content');
  await db.exec(`UPDATE skills SET display_order=3 WHERE id='${uid(900)}'; RESET ROLE;`);
  await db.exec(learningSql);
  check(await scalar(`SELECT display_order FROM skills WHERE id='${uid(900)}'`),3,'rerun retains administrator skill ordering');
  check(await scalar('SELECT count(*) FROM lesson_progress'),progressBefore,'migration preserves lesson progress');
  check(await scalar('SELECT count(*) FROM student_employability_scores'),scoresBefore,'practice setup does not alter score history');
